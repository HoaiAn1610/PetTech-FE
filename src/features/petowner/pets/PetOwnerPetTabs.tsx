...
export function VitalsTab({ pet }: { pet: PetProfile }) {
  return (
    <div className="flex flex-col gap-6">
      ...
      <SectionBox title="Chỉ số sinh tồn (Cập nhật lần cuối: 25/2)" icon={Stethoscope} iconColor="#0891b2">
...
// ─── Allergens Tab ────────────────────────────────────────────────────────────
export function AllergensTab({ pet }: { pet: PetProfile }) {
  const [allergens, setAllergens] = useState(pet.allergens);
  return (
    <div className="grid grid-cols-2 gap-6">
      <div className="flex flex-col gap-5">
        <div className="flex items-center justify-between">
          <h3 style={{ fontSize: "0.95rem", fontWeight: 800, color: "#111827" }}>Danh sách dị nguyên</h3>
          <span style={{ fontSize: "0.7rem", color: "#9ca3af" }}>Được xác minh bởi bác sĩ</span>
        </div>
        <div className="flex flex-col gap-3">
          {allergens.length === 0 ? (
            <div className="py-6 text-center">
              <Shield className="w-10 h-10 mx-auto mb-2" style={{ color: "#d1d5db" }} />
              <p style={{ fontSize: "0.85rem", color: "#9ca3af" }}>Chưa có dị nguyên nào — thú cưng có vẻ không bị dị ứng!</p>
            </div>
          ) : allergens.map(a => (
            <div key={a.id} className="rounded-xl overflow-hidden"
              style={{ border: a.severity === "severe" ? "1.5px solid rgba(124,58,237,0.3)" : a.severity === "moderate" ? "1.5px solid rgba(220,38,38,0.25)" : "1.5px solid rgba(249,115,22,0.25)" }}>
              <div className="flex items-center justify-between px-4 py-3"
                style={{ background: a.severity === "severe" ? "rgba(124,58,237,0.05)" : a.severity === "moderate" ? "rgba(220,38,38,0.04)" : "rgba(249,115,22,0.04)", borderBottom: "1px solid rgba(0,0,0,0.05)" }}>
                <div className="flex items-center gap-2">
                  <ShieldAlert className="w-4 h-4" style={{ color: a.severity === "severe" ? "#7c3aed" : a.severity === "moderate" ? "#dc2626" : "#ea580c" }} />
                  <div>
                    <span style={{ fontSize: "0.88rem", fontWeight: 700, color: "#111827" }}>{a.label}</span>
                    {a.diagnosedDate && (
                      <p style={{ fontSize: "0.62rem", color: "#9ca3af" }}>Chẩn đoán: {a.diagnosedDate}</p>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <SeverityBadge severity={a.severity} />
                  <button onClick={() => setAllergens(prev => prev.filter(x => x.id !== a.id))}
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-3 px-4 py-3.5 rounded-xl" style={{ background: "rgba(22,163,74,0.05)", border: "1px solid rgba(22,163,74,0.18)" }}>
            <div>
              ...
              <p style={{ fontSize: "0.72rem", color: "#9ca3af" }}>by {pet.diet.brand}</p>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-3">
            {[
              { label: "Calo mỗi ngày",    value: `${pet.diet.dailyCalories} kcal` },
              { label: "Bữa ăn / ngày",    value: `${pet.diet.mealsPerDay}x / ngày` },
