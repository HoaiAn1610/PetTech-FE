...
export function CartSidebar({
  ...
  return (
    <div className="w-[460px] xl:w-[500px] flex-shrink-0 h-full flex flex-col bg-white overflow-hidden border-l border-gray-100">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 bg-gray-50/30">
        {cart.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {cart.map(item => (
              <div key={item.id} className="flex items-center gap-3.5 p-3.5 rounded-2xl bg-white border border-gray-100 shadow-sm animate-in slide-in-from-bottom-1 duration-200"
                style={{ 
                  background: item.hasAllergenWarning ? "#fef2f2" : "white", 
                  borderColor: item.hasAllergenWarning ? "#fca5a5" : "#f1f5f9" 
                }}>
                <span className="text-2xl bg-gray-50 p-2 rounded-xl flex-shrink-0">{item.icon}</span>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <p className="text-sm font-extrabold text-gray-800 truncate" style={{ color: item.hasAllergenWarning ? "#991b1b" : "#1e293b" }}>
                      {item.name}
                    </p>
                    {item.hasAllergenWarning && <span title="Cảnh báo dị ứng" className="flex-shrink-0 flex items-center"><AlertTriangle className="w-4 h-4 text-red-500" /></span>}
                  </div>
                  <p className="text-xs font-semibold mt-0.5" style={{ color: item.hasAllergenWarning ? "#b91c1c" : "#64748b" }}>{formatVND(item.price)}</p>
                </div>
                <div className="flex items-center gap-2 bg-gray-50 rounded-xl p-1 border border-gray-100">
                  <button onClick={() => updateQty(item.id, -1)} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-white hover:shadow-sm text-gray-500 hover:text-red-600 transition-all">
                    <Minus className="w-4 h-4" />
                  </button>
                  <span className="text-sm font-extrabold text-gray-800 w-6 text-center">{item.qty}</span>
                  <button onClick={() => updateQty(item.id, 1)} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-white hover:shadow-sm text-gray-500 hover:text-blue-600 transition-all">
      ...
      <div className="px-4 py-4 bg-white border-t border-gray-100 shadow-[0_-4px_20px_rgba(0,0,0,0.02)]">
        ...
        {/* Totals - Dynamic Spacious heights */}
        <div className="flex flex-col gap-2 mb-4 px-4 py-3.5 rounded-2xl bg-gray-50 border border-gray-100">
          <div className="flex justify-between items-center">
            <span className="text-xs font-medium text-gray-500">Tạm tính</span>
            <span className="text-sm font-bold text-gray-700">{formatVND(subtotal)}</span>
          </div>
          {discount > 0 && (
            <div className="flex justify-between items-center">
              <span className="text-xs font-bold text-green-600">Giảm giá ({discount}%)</span>
              <span className="text-sm font-bold text-green-600">-{formatVND(discountAmt)}</span>
            </div>
          )}
          <div className="flex justify-between items-center">
            <span className="text-xs font-medium text-gray-500">Thuế (8%)</span>
            <span className="text-sm font-bold text-gray-700">{formatVND(tax)}</span>
          </div>
          <div className="flex justify-between items-center pt-2.5 mt-1 border-t border-dashed border-gray-200">
            <span className="text-base font-black text-gray-900">Tổng tiền</span>
          ...
          {[
            ...
            return (
              <button key={m.id} onClick={() => setPayMethod(m.id as any)}
                className="flex flex-col items-center justify-center gap-2 py-3.5 rounded-2xl transition-all border shadow-sm"
                style={{
                  borderColor: active ? "#2563EB" : "#f1f5f9",
                  background: active ? "#eff6ff" : "#f8fafc",
                }}>
                <Icon className={`w-5 h-5 ${active ? "text-blue-600" : "text-gray-400"}`} />
                <span className={`text-xs font-extrabold ${active ? "text-blue-700" : "text-gray-500"}`}>{m.label}</span>
              </button>
            );
          })}
        </div>
